...
public class RuntimeTest : BrowserDefault
{
    ...
    public async Task ShouldAddConnectToChrome()
    {
        ...
        await this.AssertThrowsConnect(page, "chrome.runtime.connect() called from a webpage must specify an Extension ID (string) for its first argument.", "");
        await this.AssertThrowsConnect(page, "No matching signature.", "", "", "", "", "", "", "");
        await this.AssertThrowsConnect(page, "Invalid extension id: 'foo'", "", "foo");
        await this.AssertThrowsConnect(page, "Error at property 'includeTlsChannelId': Invalid type: expected boolean, found number.", "", new { IncludeTlsChannelId = 777 });
    }
